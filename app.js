...
const init = _ =>
    {
        ...
        sortPriority.addEventListener ('click', _ => taskSort('priority')); // be _=> pasileistų funkcija užkrovus, nes su kabutėm funkcija paleidžiama
        sortCompleted.addEventListener ('click', _ => taskSort ('done'));

        TASKS = JSON.parse(localStorage.getItem('tasks')) || []; //iš local storage ARBA tuščias masyvas
                ...
                <li data-id="${id}">
                    <div class="task" data-task-view-priority="${priority}">
                        ...
                        <div class="actions">
                            ...
                            <button class="save" data-task-save>Saugoti</button>
                        </div>
                    </div>
                </li>
                `;
...
const addDeleteListener = _ =>{
    ...
    lis.forEach(li => {
        ...
        deleteButton.addEventListener('click', _ =>{
            ...
            localStorage.setItem('tasks', JSON.stringify(TASKS));
            renderTasks();
            addToast('užduotis ištrinta', 'delete');
        })
    })
}
